...
No standalone install is required; this skill demonstrates patterns using Python (pandas/matplotlib/plotly), shell, and BIDS derivatives. Ensure a working Python 3.10+ environment with `pandas`, `numpy`, `matplotlib`, and `plotly` available, plus access to your BIDS directory and QC outputs (e.g., MRIQC, fMRIPrep).
...
```bibtex
@misc{qcautomation2024,
  title={Quality control automation patterns for neuroimaging pipelines},
  author={N_tools contributors},
  year={2024},
  howpublished={\\url{https://github.com/HendricksJudy/N_tools}}
}
```
