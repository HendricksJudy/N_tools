...
Use the same installation as the main fMRIPrep pipeline (Docker/Singularity or native). No extra binaries are required; ensure fMRIPrep outputs are available along with `python` and `jq` for the QC scripts demonstrated here.
